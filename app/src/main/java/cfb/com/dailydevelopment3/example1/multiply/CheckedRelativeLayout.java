...
public class CheckedRelativeLayout extends RelativeLayout implements Checkable {
	...
	public static final int CHECK_MODE_RADIO = 0;
	public static final int CHECK_MODE_CHECK = 1;
	...
	protected void initFromAttributes(Context context, AttributeSet attrs) {
		...
		checkMode = a.getInt(R.styleable.CheckedRelativeLayout_checkmode, CHECK_MODE_RADIO);
		setChecked(checked);
		a.recycle();
	}
	...
	public boolean performClick() {
		if (checkMode == CHECK_MODE_CHECK) {
			toggle();
		} else if (checkMode == CHECK_MODE_RADIO) {
			setChecked(true);
		}
